...
get_build_installed_on_server() {
  ...
  ssh -i /root/.ssh/id_rsa -o "StrictHostKeyChecking no" root@${1} "rpm -qa | grep ${2} | sed 's/.noarch//'"
}
...
set_docker_registry(){
  ...
  ssh -i /root/.ssh/id_rsa -o "StrictHostKeyChecking no" root@${1} "${ssh_command}"
  # seems ssh -i /root/.ssh/id_rsa will omit the " as default whenever how many " we use to describe the variables
  ssh -i /root/.ssh/id_rsa -o "StrictHostKeyChecking no" root@${1} 'sed -i "s/=-/=\"-/" /etc/sysconfig/docker'
  ssh -i /root/.ssh/id_rsa -o "StrictHostKeyChecking no" root@${1} 'sed -i "s/8888$/8888\"/" /etc/sysconfig/docker'

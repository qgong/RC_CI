...
class TalkToRCCI():
  ...
  def __init__(self, username, password, build_name):
    ...
    self.server = jenkins.Jenkins(
        RC_Jenkins, username=username, password=password)
    self.test_report = []
  ...
  def get_latest_build_console_log_content(self):
    self.console_log_content = self.server.get_build_console_output(
        self.build_name, self.last_completed_build_number)

  def get_test_report_from_console_log(self):
    current_rc_version_list = re.findall(
        r'ET Version/Commit: [\w+ \.]+', self.console_log_content)
    if len(current_rc_version_list) > 0:
      self.current_rc_version = current_rc_version_list[0].split(':')[
          1].strip()
      test_type = re.findall(
          r'Testing Type: [\w+ \.]+', self.console_log_content)[0].replace("Testing Type: ", "")
      test_result = re.findall(
          r'Testing Result: [\w+ \.]+', self.console_log_content)[0].replace("Testing Result: ", "")
      test_result_url = re.findall(r'Testing Report URL: [^\n]+', self.console_log_content)[
          0].replace("Testing Report URL: ", "").replace("'", "")

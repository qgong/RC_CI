...
Perf_Jenkins = os.environ.get(
    'ET_Perf_URL') or "https://perfci.eng.pek2.redhat.com"
...
class TalktoPerfCI():
    ...
    def get_latest_build_console_log_content(self):
        self.console_log_content = self.server.get_build_console_output(
            self.build_name, self.lastest_build_number)

    def get_console_log_url(self):
        self.perf_testing_console_url = Perf_Jenkins + "/view/ET/job/" + \
            self.build_name + "/" + str(self.lastest_build_number) + "/console"

    def get_last_completed_build_number(self):
        self.last_completed_build_number = self.server.get_job_info(
            self.build_name)['lastCompletedBuild']['number']
    ...
    def run_build(self):
        ...
        self.server.build_job(self.build_name,  {
                              'ET_SERVER': ET_Perf_Server, 'Stub_Server': ET_Stub_Server, 'ET_DB': ET_DB_Server, 'et_version': self.et_rc_version,
                              'errata_fetch_brew_build': self.errata_fetch_brew_build})
    ...
    def summary_the_result(self):
        ...
        print "ET Version/Commit: " + self.et_rc_version
    ...
    def get_comparision_report_url(self):
        ...
        self.perf_testing_comparison_url = Perf_Jenkins + "/view/ET/job/" + self.build_name + "/" + str(self.lastest_build_number) \
            ...
            + str(self.lastest_build_number) + "_vs_" + \
            self.default_build_number_to_compare + "/perfcharts-simple-perfcmp"
        # print self.perf_testing_comparison_url

    def run_one_test(self):
        self.get_last_completed_build_number()
        self.run_build()
        # before get the build number, sleep some seconds to make sure the build
        # is running
        time.sleep(30)
...
if __name__ == "__main__":
    ...
    if testing_type == "smoke":
        talk_to_jenkinks_smoke = TalktoPerfCI(
            username, password, "ET_Baseline_PDI_MIN", 5, 2, baseline_job_id, et_rc_version)
    ...
    if testing_type == "all":
        ...
        talk_to_jenkinks_smoke.run_one_test()
        talk_to_jenkinks_smoke = TalktoPerfCI(
            username, password, "ET_Baseline_PDI", 80, 2, baseline_job_id, et_rc_version, errata_fetch_brew_build)
        talk_to_jenkinks_smoke.run_one_test()

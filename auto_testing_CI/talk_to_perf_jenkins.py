...
class TalktoPerfCI():
  ...
  def run_build(self):
    ...
    self.server.build_job(self.build_name,  {
                          'ET_SERVER': ET_Perf_Server, 'Stub_Server': ET_Stub_Server, 'ET_DB': ET_DB_Server})
...
if __name__ == "__main__":
  ...
  if testing_type == "full_perf":
    talk_to_jenkinks_smoke = TalktoPerfCI(
        username, password, "ET_Baseline_PDI", perf_expect_run_time, 2, baseline_job_id, et_rc_version)
    talk_to_jenkinks_smoke.run_one_test()
  if testing_type == "all":
    talk_to_jenkinks_smoke = TalktoPerfCI(
        username, password, "ET_Baseline_PDI_MIN", 5, 2, baseline_job_id, et_rc_version)
    talk_to_jenkinks_smoke.run_one_test()
    talk_to_jenkinks_smoke = TalktoPerfCI(
        username, password, "ET_Baseline_PDI", 80, 2, baseline_job_id, et_rc_version)
    talk_to_jenkinks_smoke.run_one_test()

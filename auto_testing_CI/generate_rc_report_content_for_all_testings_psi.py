...
class GenerateAllReports():
  ...
  def generate_all_reports(self):
    self.report_files_list = glob.glob(
        "*_content.txt")
    ...
    if len(self.report_files_list) == 1:
      ...
      for file in self.report_files_list:
        ...
        remove_table_head_column = remove_table_head.replace(
            self.head_column, '', 1)
        remove_table_end = remove_table_head_column.replace(
            "</tbody></table>", '', 1)
        self.general_reports_content += remove_table_end
      self.general_reports_content = "<table><tbody>" + \
          self.head_column + self.general_reports_content + "</tbody></table>"

    psi_jenkins = "https://jenkins-errata-qe-test.cloud.paas.psi.redhat.com/job/errata-qe-test"
    report_note_1 = "<p>The report is generated by <a href='" + psi_jenkins + "'> ET QE PSI Pipeline</a> automatically.</p>"
    ...
    report_footer_1 = "<p>[1] ET QE PSI pipeline is used to track all processes of build/commit testings, including enviroment preparation, testing, results collection and reports by on click. </p>"
    ...
    report_footer_5 = "<p> Anything about the reports or the testings, you can join #errata-qe or send mail to 'errata-qe-team' for help.</p>"
    report_beginning = report_note_1 + report_note_2
    report_ending = report_footer_1 + report_footer_2 + \
        report_footer_3 + report_footer_4 + report_footer_5
    self.general_reports_content = report_beginning + "<p></p>" + \
        self.general_reports_content + "<p></p>" + report_ending

...
class GenerateRCReportContent():
  ...
  def __init__(self, username, password, confluence_username, confluence_password, build_name, expected_rc_version):
    ...
    print "original test report:", self.test_report
    ...
    '''
    self.e2e_env = '''
    <p>Instant ET pods running against PSI:</p>
    <p>cucumber-et-e2e-testing.cloud.paas.psi.redhat.com</p>
    ...
    self.ts2_uat_env = '''
    <p>Instant ET pods running against PSI:</p>
    <p>cucumber-et-testing-0.cloud.paas.psi.redhat.com</p>
    <p>cucumber-et-testing-1.cloud.paas.psi.redhat.com</p>
  ...
  def generate_rc_report_for_current_rc_version(self):
    if self.test_type in ['Bug Regression Testing', 'Performance Baseline Testing'] and self.test_report != ['', '', '']:
...
if __name__ == "__main__":
  ...
  if len(sys.argv) == 7:
    ...
    file = sys.argv[7]
    generate_reprot = GenerateRCReportContent(
        username, password, confluence_username, confluence_password, build_name, et_rc_version)
